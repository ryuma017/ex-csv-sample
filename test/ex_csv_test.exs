defmodule ExCsvTest do
  ...
  describe "puts_review_avgs/1" do
    ...
    test "正常系", %{orders: orders} do
      log =
        capture_io(fn ->
          assert :ok = ExCsv.puts_review_avgs(orders)
        end)
  ...
  describe "puts_order_rates/1" do
    ...
    test "正常系", %{orders: orders} do
      log =
        capture_io(fn ->
          assert :ok = ExCsv.puts_order_rates(orders)
        end)
  ...
  defp setup_orders(_) do
    [orders: ExCsv.import!("sample.csv")]
  end
end
